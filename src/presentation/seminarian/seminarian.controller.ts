import {
  CreateForeingSeminarian,
  DeleteSeminarianUseCase,
  CreateSeminarian,
  UpdateSeminarian,
  CreateSeminarianUseCase,
  Locations_enum,
  seminarianMinistery_ENUM,
  SeminarianRepository,
  StageEnum,
  UpdateSeminarianUseCase,
  GetSeminarianDTO,
  GetSeminarianUseCase,
  seminarian_status_enum,
  GetByIDSeminarianUseCase,
  SeminarianFichaUseCase,
  GetByIDCulminadoSeminarianUseCase,
} from "../../domain";
...
export class SeminarianControler {
  ...
  public ficha = (req: Request, res: Response) => {
    new SeminarianFichaUseCase(this.repository)
  ...
  public GetConstance = async (req: Request, res: Response) => {
    new GetByIDSeminarianUseCase(this.repository)
      .execute(req.params.id)
      .then((result) => {
        const line = res.writeHead(200, {
          "Content-Type": "application/pdf",
    ...
    try {
      ...
      if (error != undefined) {
        ...
        if (get_dto != undefined) {
          new GetSeminarianUseCase(this.repository)
            .execute(get_dto)
            .then((seminarians) => {
  ...
  public delete = async (req: Request, res: Response) => {
    try {
      ...
      new DeleteSeminarianUseCase(this.repository)
        .execute(req.params.id)
        .then((result) => {
  ...
  public Create = async (req: Request, res: Response) => {
    ...
    try {
      ...
      user.role = 5;
      //assembling de seminarian
      const seminarian = new CreateSeminarian(
        foreingdata,
        user_origin.location as Locations_enum,
        user_origin.stage,
        user_origin.apostleships,
      ...
      if (errores == null) {
        new CreateSeminarianUseCase(this.repository)
          .execute(seminarian)
          .then(async (seminarian) => {
            await imageResize(req.body.ayuda);
