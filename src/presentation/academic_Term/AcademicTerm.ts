...
export class AcademicTermController {
    ...
    public Create = (req: Request, res: Response) => {
        ...
        if (academicCreateDTO != undefined) {
            ...
            .then((academic)=>{ return res.status(200).json(academic).send();})
            .catch((error) => {
                console.log(error);
                return res.status(400).json("Error creando periodos" + error).send()})
        }else{
            console.log(error_Arr);
            return res.status(400).json({ error_Arr }).send();
        } 
    }
    public Get = (req: Request, res: Response) => {
        ...
        new GetAcademicTermUseCase(this.repository).execute(get_dto).then((results) => {
            ...
            console.log(error);
            return res.status(400).json("Error obteniendo periodos" + error).send()})
    }
    ...
    public Update = (req: Request, res: Response) => {
        ...
        if (academicupdateDTO != undefined) {
            ...
            .then((academic)=>{return res.status(200).json(academic).send();})
            .catch((error) => {return res.status(400).json("Error actualizando periodos" + error).send()})
        }else{
            console.log(error_Arr);
            return res.status(400).json({ error_Arr }).send();
        } 
    ...
    public PassSemester = (req: Request, res: Response) => {
        ...
        if(id == undefined){return res.status(400).json("el id es requerido").send()}
        new PassAcademicTermSemesterUseCase(this.repository).execute(id)
        .then((academic)=>{return res.status(200).json(academic).send();})
        .catch((error) => {return res.status(400).json("Error pasando el semestre del periodo" + error).send()})
    }
    public Activate = (req: Request, res: Response) => {
        ...
        .then((academic)=>{return res.status(200).json(academic).send();})
        .catch((error) => {return res.status(400).json("Error activando el periodo" + error).send()})
    }
    public Deactivate = (req: Request, res: Response) => {
        ...
        .then((academic)=>{return res.status(200).json(academic).send();})
        .catch((error) => {return res.status(400).json("Error eliminando periodo" + error).send()})
    }
}